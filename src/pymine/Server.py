...
class Server:
    ...
    def loadLevel(name):
        ...
        if provider == None:
            self.logger.error(
                self.getLanguage(self).translateString("pymine.level.loadError", [name, "Unknown provider"]))
        ...
        try:
            ...
            self.logger.error(
                self.getLanguage(self).translateString("pymine.level.loadError", [name, e.getMessage(self)]))
            self.logger.logException(e)
            return False
    ...
    def generateLevel(self, name, seed=null, generator=null, options=dict()):
        ...
        try:
            ...
            self.logger.error(
                self.getLanguage(self).translateString("pymine.level.generateError", [name, e.getMessage(self)]))
            self.logger.logException(e)
            return False
...
def isLevelGenerated(name):
    ...
    path = self.getDataPath(self) + "worlds/" + name + "/"
    if not (self.getLevelByName(name) instanceof Level)):
...
# @return Entity|null
# ===========================================================================
def findEntity(int entityId, Level
...
# @return mixed
# ===========================================================================
def getProperty(self, variable, defaultValue=null):
    if not array_key_exists(variable, self.propertyCache)):
        v = getopt("", ["variable::"])
        if isset(v[variable])):
...
def addOp(name):
    ...
    if (player=self.getPlayerExact(name)) is not None):
        player.recalculatePermissions(self)
...
def isWhitelisted(name):
    return not self.hasWhitelist(self) or self.operators.exists(name, True) or self.whitelist.exists(name, True)
...
Server::sleeper.synchronized(function(int
ms):
...
def __construct(\ClassLoader autoloader,                            \ThreadedLogger logger, filePath, dataPath,
    ...
    try:
                                                                                                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                                                                                                     "level-name" = > "world",
                                                                                                                                                                                                                                                                                                                      "level-seed" = > "",
                                                                                                                                                                                                                                                                                                                                       "level-type" = > "DEFAULT",
                                                                                                                                                                                                                                                                                                                                                        "enable-query" = > True,
                                                                                                                                                                                                                                                                                                                                                                           "enable-rcon" = > False,
                                                                                                                                                                                                                                                                                                                                                                                             "rcon.password" = > substr(
            base64_encode(random_bytes(20)), 3, 10),
                                                                                                                                                                                                                                                                                                                                                                                                                 "auto-save" = > True,
                                                                                                                                                                                                                                                                                                                                                                                                                                 "view-distance" = > 8
        ])
        ...
        self.logger.info(self.getLanguage(self).translateString("language.selected",
                                                                [self.getLanguage(self).getName(self),
                                                                 self.getLanguage(self).getLang(self)]))

        self.memoryManager = MemoryManager(this)

        self.logger.info(
            self.getLanguage(self).translateString("pymine.server.start", [TextFormat::AQUA + self.getVersion(
        ...
        self.rcon = RCON(this,
                         self.getConfigString("rcon.password", "",
                                              ...
                                              (ip = self.getIp(self)) != "" ? ip: "0.0.0.0", self.getConfigInt(
...
def enablePlugins(type):
    ...
    def start(self):
        ...
        self.logger.info(
            self.getLanguage(self).translateString("pymine.server.defaultGameMode", [self::getGamemodeString(
